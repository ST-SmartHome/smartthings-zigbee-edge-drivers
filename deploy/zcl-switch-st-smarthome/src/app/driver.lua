...
if not child_outlets.init_child(driver,device)then
...
-- ST-SmartHome: the default handler checks the (empty) supported_capabilities
-- list and never reconfigures reporting; do both explicitly on a switch.
...
local handler=handlers[capability_id]and handlers[capability_id][command_name]
if handler ~=nil then
handlers[capability_id][command_name]=function(driver,device,command)
if child_outlets.is_child(device)then
return child_outlets.forward(driver,device,command,handler)
end
return handler(driver,device,command)
end
end
end
end

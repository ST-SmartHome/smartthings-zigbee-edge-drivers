...
local thermostat_zwt198 = {
  ...
  -- Z2M ZWT198/ZWT100-BH (tuya.ts:12581).  _TZE204_lzriup1j and _TZE204_gops3slb
  -- swap the DP4 manual/auto enum values, and _TZE200_viy9ihs7 swaps the DP104
  -- 6-1/5-2 values, so those three exacts are registered separately below.
  tuya.dp_enum(4, {
    name = "preset",
...
-- Copies a definition and replaces the mapping for one datapoint.
...
-- Z2M swaps the DP4 manual/auto enum values for these two exacts (tuya.ts:12591).
...
-- Z2M swaps the DP104 6-1/5-2 enum values for _TZE200_viy9ihs7 (tuya.ts:12624).
...
-- AVATTO ZWT100 (avatto.ts:732) shares DP1..DP19 with ZWT198 but replaces the
-- 100-block: DP103 is a plain reset, DP104 is a four-way holiday option, DP105
-- is a beeper, DP106 uses in/out/all, DP108 adds a sensor limit, DP110 has its
...
local thermostat_hy08we = {
  ...
  -- Z2M legacy.fz.hy_thermostat (legacy.ts:3284) and dataPoints.hy* (legacy.ts:736).
...
local thermostat_tervix = {
  ...
  -- Z2M reports DP3 as a working status label as well (tuya.ts:12365).
  tuya.dp_enum(3, {
    name = "working_status",
    read_only = true,
...
-- Z2M lists Beok TGR85-ZB and AVATTO ZWT-100-16A as whiteLabel retail names for
-- _TZE200_2ekuz3dz (tuya.ts:12398), not as interviewed pairs.
local thermostat_thermosphere = {
  ...
  tuya.dp_current_heating_setpoint(2, { scale = 10 }),
  -- Z2M TS0601_thermostat_thermosphere (tuya.ts:21454).  DP4 boost is an enum
  -- with OFF=1/ON=2, DP40/41/47/50/54/55 are raw and DP16 schedule plus DP48
  -- temp_tolerance stay unexposed as Z2M leaves them commented out.
  tuya.dp_enum(4, {
    name = "boost",
...
local thermostat_bot_r9v = {
  ...
  -- Z2M derives both the running state and a valve label from DP36 (tuya.ts:26352).
  tuya.dp_binary(36, {
    name = "valve_state",
    read_only = true,
...
local thermostat_te_1z = {
  ...
  -- Z2M sensor_mode keys are lowercase in/ou/al (tuya.ts:19974).
  tuya.dp_enum(116, {
    name = "sensor_mode",
  ...
  -- Z2M preset is manual(0)/auto(1)/mixed(3); the zero value was named heat.
  tuya.dp_enum(128, {
    name = "preset",
...
local thermostat_pilot_wire = {
  ...
  -- Z2M reads DP19 raw here (tuya.ts:24454), not divided by ten.
...
-- Z2M lists TH-P1Z, TH-P0Z and MZTE1Z as whiteLabel retail names attached to the

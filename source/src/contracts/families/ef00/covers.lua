-- 커버/블라인드 디바이스 정의
-- ZHA ts0601_cover.py + Z2M zigbee-herdsman-converters 기반 DP 그룹핑
--
-- 현재는 핵심 커버 제어(open/close/pause + position)만 우선 지원합니다.
-- 캘리브레이션, 행정/중간 리밋, 배터리, 추가 모터 옵션은 기기별 편차가 커서
-- core capability와 직접 연결되는 DP만 먼저 등록합니다.
...
-- Z2M coverAction reports OPEN/STOP/CLOSE/CONTINUE through the same DP used
-- for commands. SmartThings has no STOP/CONTINUE shade state, so those two
-- reports are represented as the non-terminal "partially open" state until a
-- position report supplies the exact level.
...
-- TS0601_cover_1 / TS0601_cover_2 / TS0601_cover_3 / TS0601_cover_4 /
-- TS0601_cover_5 / TS0601_cover_6 / TS0601_cover_7 / TS0601_cover_8 /
-- TS0601_cover_9 / TS0601_cover_12 / Zemismart ZM25RX-08/30
local cover_core = {
  tuya.dp_enum(1, {
    name = "cover_state",
    converter = cover_state_standard,
...
-- TS0601_cover_6 / TS0601_cover_8 / M3TYW-2.0-13
...
-- TS0601_cover_6: cpbo62rn/libht6ua revisions, audited independently.
local cover_six = {
  ...
  -- DP107 SET/raw wire contract is unresolved upstream.
}
...
-- TS130F 외형이지만 Window Covering attribute가 아니라 EF00 DP로 동작한다.
-- Z2M exact 정의의 모든 wire DP를 보존하되, 현재 profile에는 핵심 cover만
-- 노출하고 설정 DP는 family-specific capability 감사 전까지 내부 매핑으로 둔다.
...
-- Z2M Novato ZSM-01 (tuya.ts:1978): DP1 state, DP9 set position, DP8 position
-- report, DP11 motor direction forward/back, DP19 preferred position 0..100%
-- and DP20 step control up/down.  The last three were parsed but never
-- exposed, so the dedicated profile now carries them.
...
local cover_zb_sm = {
  ...
  -- Z2M legacy fz.zb_sm_cover (legacy.ts:1503) treats DP3 as the arrived
  -- position report for the same shade level, so it stays internal rather than
  -- becoming a second exposed position.
  ...
  -- Z2M maps DP121 as 0 opening / 1 stopped / 2 closing (legacy.ts:1568).
  tuya.dp_enum(121, {
    name = "motor_state",
    read_only = true,
...
-- Moes ZC-LS02 koxaopnk: inverted position reports on both DP2 and DP3.
-- Battery is only reported after a query; retain daily and announce queries.
-- DP5 direction is withheld: upstream explicitly questions BOOL vs ENUM.
local moes_zcls02 = {
...
-- TS0601 cover10 reports position separately on DP3, unlike cowvfni3.
local cover_ten = {
  ...
  -- DP5 direction is withheld pending resolution of the upstream BOOL/ENUM note.
}
...
-- PIMS3028 / cover plug-in receiver multi STAK 3/STAS 3
local cover_core_pims3028 = {
  ...
  -- Z2M maps DP3 to the same inverted position report as DP2 (tuya.ts:25832),
  -- so it stays internal instead of becoming a second exposed position.
...
-- Z2M Ronco RM28-LE (tuya.ts:25313): DP1 state, DP2/DP3 position, DP4 mode
-- morning/night, DP5 motor direction, DP6 auto power, DP7 work state,
-- DP8 countdown cancel/1h..4h, DP9 countdown left, DP10 full travel time,
-- DP11 situation set, DP12 motor fault (non-zero), DP13 battery, DP16 border
-- with five values, DP19 favourite position and DP20 jog step.  DP4, DP8,
-- DP9, DP10, DP11 and DP19 were missing entirely and the rest were hidden.
local cover_rm28_le = {
  ...
  -- Z2M reports any non-zero DP12 value as a fault (tuya.ts:25398).
  tuya.dp_numeric(12, {
    name = "motor_fault",
    read_only = true,
...
local cover_model_zm79e_dt = {
  ...
  -- Z2M inverts these enum values: start is 1 and stop is 0 (tuya.ts:8451).
  tuya.dp_enum(102, {
...
-- MB60L-ZG-ZT-TY battery blind motor
local cover_model_mb60l = {
  ...
  -- Z2M puts child lock on DP101 and the step/tilt mode on DP103
  -- (tuya.ts:8541); the previous DP108/DP109 pair did not match this exact.
  tuya.dp_binary(101, {
    name = "child_lock",
...
-- EPJ-ZB / smart sliding window pusher
local cover_epj_zb = {
...
-- ZHC26.107 TS0301_cover_1: only this explicit exact has the DP5/13 contract.

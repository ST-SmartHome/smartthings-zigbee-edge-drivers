-- Wave19 FrankEver/Lidl valve source-only candidates.
-- Frozen Zigbee2MQTT v26.99.0:
--   src/devices/frankever.ts:33-132
--   src/devices/lidl.ts:41-222,404-531
...
lidl.configure = function(driver, device)
  ...
  if not get_field(device, LIDL_RESET_SCHEDULED_FIELD)
    ...
    device.thread:call_with_delay(10, function()
      -- The public reset command is ENUM(0), but the frozen interview hook is
      -- a distinct sendDataPointBool(109, false) followed by BOOL(108, false).
